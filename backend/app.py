...
async def generate(sid, data):
    ...
    try:
        ...
        if temperature is None or temperature < 0 or temperature > 3:
            ...
            logger.warning(
                f"Invalid temperature provided. Using default: {DEFAULT_TEMP}"
            )
...
async def stream_response(sid, agent_id, query_d):
    ...
    # print("\n\n\n\n\n\nSTREAM_RESPONSE")
    # pprint(query_d)
    max_tokens = query_d.get("max_tokens", DEFAULT_MAX_TOKENS)
    try:
        async for i, response in enumerate(stream_llm_response(**query_d)):
            model_output += response
            print(response, end="", flush=True)
            progress = int((i + 1) / max_tokens * 100)
            await sio.emit(
                "response",
                {
                    "model": query_d["model"],
        ...
        await sio.emit(
            "response_complete",
            {"model": query_d["model"], "agentId": agent_id},
            to=sid,
        )
...
async def fetchRepoContent(sid, data):
    ...
    try:
        ...
        await sio.emit(
            "repoContentStarted", {"message": "Fetching repo content..."}, room=sid
        )
    except Exception as e:
